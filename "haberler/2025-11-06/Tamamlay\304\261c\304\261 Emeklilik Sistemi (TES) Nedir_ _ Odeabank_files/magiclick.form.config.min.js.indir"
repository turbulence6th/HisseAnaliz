...
var form={select2:function(){window.innerWidth>990?$(".select2").each(function(){var n=$(this).data("placeholder");$(this).select2({width:"100%",minimumResultsForSearch:10,maximumSelectionLength:3,dropdownCssClass:$(this).parent().hasClass("select-box--investment")?"select2-dropdown-investment":"",placeholder:n}).on("select2:close",function(){$(this).find("option:selected").val()==""&&$(this).parents(".form-item").removeClass("active")})}):$("select").each(function(){$(this).data("select2")&&$(this).select2("destroy")});$(document).on("select2:open",function(){document.querySelector(".select2-search__field").focus()})},init:function(){form.select2();$(window).resize(function(){form.select2()})}};form.init();